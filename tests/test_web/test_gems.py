"""tests/test_web/test_gems.py — /gems ルートテスト"""
...
from rails_lens.models import GemIntrospectOutput
...
def _make_client(monkeypatch: pytest.MonkeyPatch) -> TestClient:
    ...
    monkeypatch.setattr(
        "rails_lens.web.routes.gems.gem_introspect_impl",
        AsyncMock(return_value=GemIntrospectOutput(model_name="")),
    )
    monkeypatch.setattr(
...
def test_get_gems(monkeypatch: pytest.MonkeyPatch) -> None:
    client = _make_client(monkeypatch)
    response = client.get("/gems")
    assert response.status_code == 200


def test_get_gem_detail(monkeypatch: pytest.MonkeyPatch) -> None:
    client = _make_client(monkeypatch)
    response = client.get("/gems/devise")
    assert response.status_code == 200
