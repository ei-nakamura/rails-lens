...
Phase 4: 品質チェック
├── n_plus_one_detector   → パフォーマンス問題の事前検出（ループ内の重複）
├── redundancy_detector   → パフォーマンス問題の事前検出（処理フロー上の重複）
...
| Phase 4 | 実装済みコード + Phase 1 のspec | N+1検出結果、冗長呼び出し検出結果、セキュリティ問題、一貫性レポート | 修正タスクのバックログ |
...
| Python | `tools/response_shape_suggest.py`, `tools/api_audit.py`, `tools/n_plus_one_detector.py`, `tools/redundancy_detector.py`, `analyzers/serializer_parser.py`, `analyzers/view_attribute_extractor.py`, `analyzers/api_convention_detector.py`, `analyzers/n_plus_one_analyzer.py`, `analyzers/redundancy_analyzer.py` |
| モデル | `models.py` に入出力モデルを追加 |
| テスト | `test_response_shape_suggest.py`, `test_api_audit.py`, `test_n_plus_one_detector.py`, `test_redundancy_detector.py` |
...
- F-5（redundancy_detector）は F-3 と補完関係にあり、同一フェーズで実装することで品質チェックの網羅性を確保できる。既存ツールの出力を統合するオーケストレーションが主体のため、Phase 10 の他ツール完了後に着手する
...
- `redundancy_detector` がメソッド単位（モード1）とエンドポイント全体（モード2）で冗長な呼び出しを検出し、フェーズ別コスト分解を出力する
...
| Phase 10 | F-1, F-2, F-3, F-5 | 1本 | 4ツール + 5アナライザ | M + M + M + L |
...
既存ツール:
  get_routes      → endpoint_inventory, api_audit, generate_openapi, before_action_chain,
                     n_plus_one_detector, redundancy_detector, exposure_check
  introspect_model → generate_openapi, generate_pydantic_models, response_shape_suggest,
                     n_plus_one_detector, redundancy_detector, exposure_check,
                     view_model_coupling, service_extraction_map
  find_references  → response_shape_suggest, api_audit, n_plus_one_detector,
                     redundancy_detector, view_model_coupling, service_extraction_map,
                     logic_extract
  trace_callback_chain → logic_extract, redundancy_detector
  data_flow        → generate_openapi, generate_pydantic_models, redundancy_detector
...
新規ツール間:
  ...
  before_action_chain    → redundancy_detector（フィルタチェーン内の呼び出し把握）
  n_plus_one_detector    ↔ redundancy_detector（補完関係: ループ内重複 vs 処理フロー上重複）
```
